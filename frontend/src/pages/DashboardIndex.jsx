...
const DashboardIndex = () => {
  ...
  return (
    <div className="min-h-full flex flex-col items-center justify-center p-6 md:p-12 max-w-3xl mx-auto text-center">
      ...
      <p className="text-sm md:text-base text-zinc-500 dark:text-zinc-400 max-w-md mb-8 leading-relaxed">
      ...
      <button
        ...
        disabled={isCreating}
        className="inline-flex items-center space-x-2 px-5 py-2.5 text-sm font-semibold rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg transition-all"
      >
        {isCreating ? (
          <>
      ...
      {notes && notes.length > 0 && (
        <div className="mt-12 w-full text-left pt-8 border-t border-zinc-200/80 dark:border-zinc-800/80">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {notes.slice(0, 4).map((note) => (
              <button
                ...
                className="flex items-start space-x-3 p-3 rounded-lg border border-zinc-200/70 dark:border-zinc-800/70 bg-zinc-50/50 dark:bg-zinc-900/50 hover:bg-zinc-100/70 dark:hover:bg-zinc-800/70 text-left transition-colors group"
              >
                <span className="text-lg select-none">📄</span>
                <div className="min-w-0 flex-1">
                  <h3 className="text-sm font-medium text-zinc-800 dark:text-zinc-200 truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
